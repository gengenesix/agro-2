...
export default function DealerWalletPage() {
  ...
  const inEscrow = txns.length > 0
    ...
    : orders
        .filter(o => ['pending', 'confirmed', 'preparing'].includes(o.trackingStatus))
    ...
    <main className="min-h-screen bg-cream pb-10">
      <div className="bg-white border-b border-border sticky top-0 z-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4">
          ...
          <p className="text-xs text-muted-foreground">Revenue from input sales</p>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        {inEscrow > 0 && (
          <div className="bg-sector-inputs-bg border border-sector-inputs/20 rounded-2xl px-5 py-4 flex justify-between items-center">
            <div>
              <p className="text-xs font-bold text-sector-inputs uppercase tracking-wider">Awaiting Dispatch</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                Held until orders are delivered and confirmed
              </p>
            </div>
          ...
          {txns.filter(tx => Number(tx.amount) > 0).length === 0 ? (
            <div className="py-10">
              <EmptyState
                ...
                description="Payments from input orders will appear here once buyers complete purchases."
              />
            </div>
          ) : (
