...
const STATUS_CONFIG: Record<string, { label: string; cls: string }> = {
  ...
  preparing:  { label: 'Preparing',  cls: 'bg-lime/10 text-forest' },
...
export default function DealerOrdersPage() {
  ...
  const filtered = orders.filter(o => {
    if (tab === 'active')    return !['cancelled', 'disputed'].includes(o.trackingStatus) && !o.completedAt
    if (tab === 'completed') return !!o.completedAt
    return true
  })
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-5 space-y-3">
        {loading ? (
          ...
          <EmptyState
            ...
            title="No orders"
            description="Orders from buyers and farmers will appear here."
          />
        ) : (
          filtered.map(order => {
            const effectiveStatus = order.completedAt ? 'completed' : (order.trackingStatus ?? 'pending')
            ...
            return (
              <div key={order.id}
                ...
                <Link href={`/dealer/orders/${order.id}`} className="flex items-center gap-4 p-4">
                ...
                {order.trackingStatus === 'pending' && (
                  <div className="border-t border-border px-4 py-3 bg-cream/30 flex gap-2">
                    <button
                      ...
                      onClick={() => advanceStatus(order.id, 'confirmed')}
                ...
                {order.trackingStatus === 'confirmed' && (
                  <div className="border-t border-border px-4 py-3 bg-cream/30 flex gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-lime text-forest
                                 rounded-lg hover:bg-lime-dark transition-colors disabled:opacity-60">
                      <CheckIcon size={12} /> Mark preparing
                    </button>
                  </div>
                )}
                {order.trackingStatus === 'preparing' && (
                  <div className="border-t border-border px-4 py-3 bg-cream/30 flex gap-2">
                    <button
                      disabled={acting === order.id}
                      onClick={() => advanceStatus(order.id, 'dispatched')}
