...
import {
  SearchIcon, MapPinIcon, InputsIcon, PayAtHarvestIcon,
    ...
    <main className="min-h-screen bg-cream pb-24">
      ...
      <div className="bg-white border-b border-border sticky top-0 z-30">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4">
          <h1 className="font-bold text-forest text-lg mb-3">Inputs Market</h1>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-5">
        <p className="text-xs text-muted-foreground mb-4">
          {loading ? 'Loading…' : fetchError ? '' : `${total.toLocaleString('en-GH')} input${total !== 1 ? 's' : ''} available`}
        </p>
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-border p-8 text-center">
            <p className="text-sm font-semibold text-forest">Could not load inputs</p>
                ...
                return (
                  <Link key={item.id} href={`/farmer/inputs/${item.slug ?? item.id}`}
                    ...
                    <div className="relative aspect-[4/3] bg-cream-dark overflow-hidden">
                      ...
                      {item.bnplAvailable && (
                        <div className="absolute top-2 right-2"><BnplBadge /></div>
                      )}
